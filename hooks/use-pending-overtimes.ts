...
import { fetchApprovedDeniedOvertimes, fetchPendingOvertimes } from '@/data/fetch-pending-overtimes'
...
export function useApprovedDeniedOvertimes() {
    const [overtimes, setOvertimes] = useState<OvertimesWithUser[] | null>(null)
    const [isLoading, setIsLoading] = useState(true)
    const [error, setError] = useState<string | null>(null)
    ...
    useEffect(() => {
        const fetchOvertimes = async () => {
            ...
            try {
                const result = await fetchApprovedDeniedOvertimes()
