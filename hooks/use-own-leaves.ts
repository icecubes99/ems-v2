...
import { fetchOtherLeaves, fetchOwnLeaves } from '@/data/fetch-own-leaves'
...
export function useOtherLeaves(userId: string) {
    const [leaves, setLeaves] = useState<Leaves[] | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    ...
    useEffect(() => {
        const fetchLeaves = async () => {
            ...
            try {
                const result = await fetchOtherLeaves(userId);
