...
export const deleteMultipleDeductionsOfUser = async (deductionsIds: string[]) => {
    const user = await currentUser();
    if (!user) {
        return { error: "Unauthorized!" };
    }
    ...
    await auditAction(dbUser.id, `Multiple Deductions Deleted by Admin: ${adminName}`);

    await db.$transaction(async (tx) => {
        for (const deductionsId of deductionsIds) {
            await tx.deductions.delete({
                where: { id: deductionsId }
            });
        }
    });
